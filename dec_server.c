...
int main(int argc, char *argv[]){
  ...
  // bufers for storing the plaintext, keytext and ciphertext
  char *plaintext = NULL;
  char *keytext = NULL;
  ...
  while(1){
    ...
    // maintains the maximum concurrent processes at 5
    ...
    //---------------FORKING----------------------------
    // NEED TO HAVE A CHECK THAT YOU'RE NOT GOING OVER 5 PROCESSES
    ...
    switch(spawnPid){
      ...
      case 0:
        ...
        if (strcmp(buffer, "dec_client")!=0){
          ...
          // send rejection token with "sendall" functionality
          while(totalTrans < 6){
        ...
        // receives the length of the plaintext. If data-loss, continues receiving 
        // until all expected characters are received
        ...
        // STEP 2 RECEIVE PLAINTEXT FILE OF THAT LENGTH
        
        // create buffer allocation for plaintext and ciphertext
        ...
        // receives the entire plaintext file in a buffer. If a connection is
        ...
        // allocates memory for the keytext
        ...
        while (plaintext[i] != '\0'){
          char keyIdx;
          int plainIdx;
          // sets the index of the key. Special case if it is a space,
          // otherwise, use ASCII value to calculate
          if (keytext[i]==32){
            keyIdx = 26;
          } else{
            keyIdx = (keytext[i] - 65);
          ...
          if (plaintext[i]!=32 && !(plaintext[i] >= 65 && plaintext[i] <= 90)){
            ...
            // decrypts the plaintext character with the key's character
            // Subtracts the key character's idx from the plaintext idx
            // Add 27 if result is under 0 (to imitate modulo operation)
            // This produces the index of the decrypted-cipher's character
            if (plaintext[i]==32) {
              plainIdx = (26 - keyIdx);
              if (plainIdx < 0) plainIdx += 27;
              ciphertext[i] = allowed[plainIdx];
            } else { 
              plainIdx = (plaintext[i] - 65) - keyIdx;
              if (plainIdx < 0) plainIdx += 27;
              ciphertext[i] = allowed[plainIdx];
            }
          }
