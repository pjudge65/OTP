...
int main(int argc, char *argv[]) {
  ...
  long totalTrans, bytesRemaining;
  size_t plainReadLen;
  size_t keyReadLen;
  int temp_len = 0;
  char *temp_str = NULL;
  ...
  //If it wasn't validated by server, terminate immediately
  memset(buffer, '\0', sizeof(buffer));
  totalTrans = 0;
  bytesRemaining = 6;
  while (totalTrans < 6){
    charsRead = recv(socketFD, buffer+totalTrans, bytesRemaining, 0);
    if (charsRead < 0){
  ...
  // IF REJECTED, TERMINATE THE CLIENT
  ...
  //plainBuffer[plainReadLen++] = '\0';
  plainBuffer[strcspn(plainBuffer, "\n")] = '\0';
  
  //plainLength is just the amount of characters without the null terminator
  plainLength -= 1;
  plainReadLen -= 1;
  fclose(plainFile);

  // IF CLIENT RECEIVES BAD CHARACTERS, 
  ...
  // IF KEY FILE SHORTER THAN PLAINTEXT
  // TERMINATE PROGRAM, ERROR TO STDERR AND EXIT w/ VALUE 1
  if ((keyLength-1) < plainLength){
    fprintf(stderr, "CLIENT: ERROR: Length of key files less than plaintext");
    exit(1);
  }
  ...
  // sends the plaintext length as a string to the server. Continues
  // sending if network drops data send
  totalTrans = 0;
  bytesRemaining = 255;
  ...
  // sends the entire plaintext file to the server
  ...
  // clears out the plaintext buffer to receive the ciphertext
